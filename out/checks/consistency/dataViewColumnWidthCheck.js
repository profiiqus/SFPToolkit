...
class ColumnWidthCheck extends baseCheck_1.BaseCheck {
    check(document) {
        ...
        if (rootElement !== 'DataView')
            return diagnostics; // Only process DataView root elements
        const dataSourceRegex = /<DataSource>([\s\S]*?)<\/DataSource>/g;
        let dataSourceMatch;
        while ((dataSourceMatch = dataSourceRegex.exec(text)) !== null) {
            const dataSourceText = dataSourceMatch[1];
            const columnsRegex = /<Columns>([\s\S]*?)<\/Columns>/g;
            let columnsMatch;
            while ((columnsMatch = columnsRegex.exec(dataSourceText)) !== null) {
                ...
                while ((columnMatch = columnRegex.exec(columnsText)) !== null) {
                    ...
                    if (!widthMatch) {
                        const startPos = document.positionAt(dataSourceMatch.index + columnsMatch.index + columnMatch.index);
                        const endPos = document.positionAt(dataSourceMatch.index + columnsMatch.index + columnMatch.index + columnText.length);
...
//# sourceMappingURL=dataViewColumnWidthCheck.js.map